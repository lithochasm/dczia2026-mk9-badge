...
_EDGE = tuple(0.08 if led >= BACKLIGHT_START else 0.0 for led in range(NUM_PIXELS))
...
def render_theme(frame, theme, seconds, tilt_x=0.0, tilt_y=0.0, sparkle=None):
    ...
    if theme == 0:
        ...
        time_phase = seconds * 1.05 + tilt_y * 1.3
        second_phase = seconds * 0.52
    elif theme == 3:
        time_phase = seconds * 2.7
    elif theme == 4:
        time_phase = seconds * 0.72
    elif theme == 5:
        time_phase = seconds * 1.5
        second_phase = seconds * 0.15
    elif theme == 6:
        time_phase = seconds * 0.68 + tilt_y
    ...
    for led in range(NUM_PIXELS):
        ...
        if theme == 0:  # Prism: flowing diagonal rainbow
            ...
            spiral = (_ANGLE[led] / 6.28318 + _RADIUS[led] * 0.20 - second_phase + tilt_x * 0.08)
            pulse = 0.5 + 0.5 * math.sin(time_phase - _RADIUS[led] * 1.2)
            color = scale(palette(_ULTRA, spiral), 0.17 + pulse * 0.34 + edge)

        elif theme == 6:  # Sunset horizon
            horizon = 0.5 + 0.5 * math.sin(time_phase + x * 0.45 - y * 0.9)
            shimmer = 0.5 + 0.5 * math.sin(second_phase + _RADIUS[led] * 1.8)
            color = scale(palette(_SUNSET, 0.16 + horizon * 0.70), 0.21 + shimmer * 0.25 + edge)

        elif theme == 7:  # Glacier breathing rings
            ring = 0.5 + 0.5 * math.sin(time_phase - _RADIUS[led] * 1.55)
            crest = ring * ring
